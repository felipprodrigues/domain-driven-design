...
export class PatientController {
  ...
  async createPatient(req, res) {
    try {
      ...
      const createPatient = await this.patientService.addPatient(patient);
  ...
  async getPatientsByBloodType(req, res) {
    try {
      const { bloodType } = req.params;
      const patients =
        await this.patientService.findPatientByBloodType(bloodType);
      if (!patients || patients.length === 0) {
